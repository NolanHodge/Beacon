package com.comp3004.beacon.Networking;

import com.comp3004.beacon.User.CurrentBeaconUser;

/**
 * Created by julianclayton on 16-10-01.
 * This class contains all the headers for messages.
 */
public class MessageTypes {

    public static final String NOTIFICATION_REQUEST_MESSAGE = "notificationRequest";
    public static final String BEACON_REQUEST_MESSAGE = "beaconRequest";
    public static final String PUBLIC_BEACON = "publicBeacons";
    public static final String REGISTER_USER_MESSAGE = "beaconUsers";
    public static final String FRIENDS_MESSAGE = "friends";
    public static final String MESSAGE_REQUEST = "messageRequests";
    public static final String LOCATION_REQUEST = "locationRequests";
    public static final String FRIEND_REQUEST = "friendRequests";
    public static final String FRIEND_REQUEST_ACCEPT = "friendRequestAccept";


}
